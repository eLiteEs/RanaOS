...
FramebufferInfo VGraphics::fbInfo;

// font8x16 by hubenchang0515
...
void VGraphics::initialize(FramebufferInfo* fbInfo) {
    VGraphics::fbInfo = *fbInfo;
}

void VGraphics::putPixel(uint32_t x, uint32_t y, Color color) {
    if (x >= fbInfo.width || y >= fbInfo.height) return;
    
    uint32_t* pixel = (uint32_t*)((uintptr_t)fbInfo.address + (y * fbInfo.pitch) + (x * (fbInfo.bpp / 8)));
    *pixel = (color.a << 24) | (color.r << 16) | (color.g << 8) | color.b;
}

void VGraphics::drawChar(uint32_t x, uint32_t y, char c, Color fg, Color bg) {
...
void VGraphics::drawString(uint32_t x, uint32_t y, const char* str, Color fg, Color bg) {
    uint32_t currentX = x;
    while (*str) {
        if (*str == '\n') {
            y += 16;
...
int VGraphics::getWidth() {
    return fbInfo.width;
}

int VGraphics::getHeight() {
    return fbInfo.height;
}

int VGraphics::getPitch() {
    return fbInfo.pitch;
}